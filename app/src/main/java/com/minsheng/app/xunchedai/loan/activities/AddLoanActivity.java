...
public class AddLoanActivity extends BaseActivity implements View.OnClickListener, AdapterView.OnItemClickListener{
    ...
    @ViewInject(R.id.add_agreement_entrust)
    private LinearLayout ll_add_agreement_entrust;
    @ViewInject(R.id.add_agreement_transfer)
    private LinearLayout ll_add_agreement_transfer;
    @ViewInject(R.id.add_license)
    private LinearLayout ll_add_license;
    @ViewInject(R.id.add_agreement_apply)
    private LinearLayout ll_add_agreement_apply;
    ...
    @ViewInject(R.id.upload_agreement_apply)
    private TextView tv_upload_agreement_apply;
    @ViewInject(R.id.upload_agreement_entrust)
    private TextView tv_upload_agreement_entrust;
    @ViewInject(R.id.upload_agreement_transfer)
    private TextView tv_upload_agreement_transfer;
    ...
    @ViewInject(R.id.select_loan_object)
    private RelativeLayout rl_select_loan_object;
    ...
    @ViewInject(R.id.loan_object)
    private TextView tv_loan_object;
    private HttpUtils httpUtils;
    private String loan_id;
    private Loan loan;

    private String str_name, str_phone, str_warranty, str_idcard1, str_idcard2, str_bankcard1, str_bankcard2, str_license, str_agreement_apply, str_agreement_entrust, str_agreement_transfer, str_insurance_amount, str_protection_date, str_installment, str_loan_amount , str_loan_object, str_invoice, str_care_card, str_reason;
    ...
    public void showView() {
        if (is_new) {
            ...
            rl_select_loan_object.setOnClickListener(this);
            state = STATE_NEW;
            if (is_from_index) {
    ...
    public void queryDetail()
    {
        ...
        httpUtils.send(HttpRequest.HttpMethod.POST, WebServiceUrl.WEBSERVICE_URL + WebServiceUrl.LOAN_DETAIL_URL, params, new RequestCallBack<String>() {
            ...
            public void onSuccess(ResponseInfo<String> responseInfo) {
                ...
                str_loan_object = loan.getLoan_object();
                ...
                tv_protection_date.setText(str_protection_date);
                tv_installment.setText(str_installment);
                tv_loan_object.setText(loanObjectString(str_loan_object));
                tv_loan_amount.setText("可贷款额度：￥ " + str_loan_amount);
                ...
                str_agreement_apply = loan.getAgreement_apply();
                str_agreement_entrust = loan.getAgreement_entrust();
                str_agreement_transfer = loan.getAgreement_transfer();
     ...
     * 放款对象名称的转换
     */
    public String loanObjectString (String str) {
        String re;
        switch(str) {
            case "0":
                re = "借款人";
    ...
    public void showDraft() {
        ...
        if (!TextUtils.isEmpty(loan.getLoan_object())) {
            str_loan_object = loan.getLoan_object();
            tv_loan_object.setText(loanObjectString(str_loan_object));
            tv_loan_object.setTextColor(ContextCompat.getColor(getBaseContext(), R.color.gray_3));
        }
        ...
        if (!TextUtils.isEmpty(loan.getAgreement_apply())) {
            tv_upload_agreement_apply.setText("已上传");
            tv_upload_agreement_apply.setTextColor(ContextCompat.getColor(this, R.color.qing));
            str_agreement_apply = loan.getAgreement_apply();
        }

        if (!TextUtils.isEmpty(loan.getAgreement_entrust())) {
            tv_upload_agreement_entrust.setText("已上传");
            tv_upload_agreement_entrust.setTextColor(ContextCompat.getColor(this, R.color.qing));
            str_agreement_entrust = loan.getAgreement_entrust();
        }
        if (!TextUtils.isEmpty(loan.getAgreement_transfer())) {
            tv_upload_agreement_transfer.setText("已上传");
            tv_upload_agreement_transfer.setTextColor(ContextCompat.getColor(this, R.color.qing));
            str_agreement_transfer = loan.getAgreement_apply();
        }
    ...
    public void setFilesTip(String tip) {
        ...
        tv_upload_agreement_apply.setText(tip);
        tv_upload_agreement_entrust.setText(tip);
        tv_upload_agreement_transfer.setText(tip);
    }
    ...
    public Boolean checkData() {
        ...
        if (TextUtils.isEmpty(str_agreement_apply)) {
            T.show(this, "借贷申请协议不得为空");
            return false;
        }
        if (TextUtils.isEmpty(str_agreement_entrust)) {
            T.show(this, "三方协议不得为空");
            return false;
        }
        if (TextUtils.isEmpty(str_agreement_transfer)) {
            T.show(this, "划扣协议不得为空");
            return false;
        }
    ...
    public void saveLoan(DbUtils db) {
        ...
        loan.setAgreement_apply(str_agreement_apply);
        loan.setAgreement_entrust(str_agreement_entrust);
        loan.setAgreement_transfer(str_agreement_transfer);
        ...
        if ("请选择".equals(str_loan_object)) {
            str_loan_object = "";
        }
        loan.setLoan_object(str_loan_object);
    ...
    public void addApply() {
        ...
        params.addBodyParameter("loan_object", str_loan_object);
        ...
        params.addBodyParameter("agreement_apply", str_agreement_apply);
        params.addBodyParameter("agreement_entrust", str_agreement_entrust);
        params.addBodyParameter("agreement_transfer", str_agreement_transfer);
    ...
    public boolean isDraft() {
        ...
        str_protection_date = tv_protection_date.getText().toString();
        str_installment = tv_installment.getText().toString();
        str_loan_object = tv_loan_object.getText().toString();
        if( TextUtils.isEmpty(str_name) &&
                ...
                TextUtils.isEmpty(str_agreement_apply) &&
                TextUtils.isEmpty(str_agreement_entrust) &&
                TextUtils.isEmpty(str_agreement_transfer)
                )
        {
            return false;
        } else {
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.add_license:
                ...
                break;
            case R.id.add_agreement_apply:
                bundle.putInt("file", UploadActivity.FILE_AGREEMENT_APPLY);
                bundle.putString("imgUrl", str_agreement_apply);
                intent.putExtras(bundle);
                startActivityForResult(intent, UploadActivity.FILE_AGREEMENT_APPLY);
                break;
            case R.id.add_agreement_entrust:
                bundle.putInt("file", UploadActivity.FILE_AGREEMENT_ENTRUST);
                bundle.putString("imgUrl", str_agreement_entrust);
                intent.putExtras(bundle);
                startActivityForResult(intent, UploadActivity.FILE_AGREEMENT_ENTRUST);
                break;
            case R.id.add_agreement_transfer:
                bundle.putInt("file", UploadActivity.FILE_AGREEMENT_TRANSFER);
                bundle.putString("imgUrl", str_agreement_transfer);
                intent.putExtras(bundle);
                startActivityForResult(intent, UploadActivity.FILE_AGREEMENT_TRANSFER);
                break;
            ...
            case R.id.select_installment:
                ...
                break;
            case R.id.select_loan_object:
                initPopupWindowObject(v);
                break;
            case R.id.cancel:
            ...
            case R.id.borrower:
                str_loan_object = "0";
                tv_loan_object.setText(loanObjectString(str_loan_object));
                tv_loan_object.setTextColor(ContextCompat.getColor(this, R.color.gray_3));
                PopupWindowUtils.destroy(popupWindow);
                break;
            case R.id.salesman:
                str_loan_object = "1";
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                ...
                case UploadActivity.FILE_LICENSE:
                    ...
                    break;
                case UploadActivity.FILE_AGREEMENT_APPLY:
                    tv_upload_agreement_apply.setText("已上传");
                    tv_upload_agreement_apply.setTextColor(ContextCompat.getColor(this, R.color.qing));
                    str_agreement_apply = data.getStringExtra("path");
                    break;
                case UploadActivity.FILE_AGREEMENT_ENTRUST:
                    tv_upload_agreement_entrust.setText("已上传");
                    tv_upload_agreement_entrust.setTextColor(ContextCompat.getColor(this, R.color.qing));
                    str_agreement_entrust = data.getStringExtra("path");
                    break;
                case UploadActivity.FILE_AGREEMENT_TRANSFER:
                    tv_upload_agreement_transfer.setText("已上传");
                    tv_upload_agreement_transfer.setTextColor(ContextCompat.getColor(this, R.color.qing));
                    str_agreement_transfer = data.getStringExtra("path");
                    break;
                default:
                    break;
            }
